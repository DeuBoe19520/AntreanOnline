...
public class AntreLangsungActivity extends AppCompatActivity implements OnClickListener {
    ...
    private void finalUpload() {
        for (int i = 0; i < uriList.size(); i++) {
            ...
            StorageReference fileReference = mStorageReference.child("M" + key + i + "." + getFileExtension(praFile));
...
//                    mStorageReference = mFirebaseStorage.getReference(LANGSUNG_DATABASE).child(key).child(fileName);
//                    mStorageReference.putFile(mUri).addOnCompleteListener(task -> {
//                        String s = task.getResult().toString();
//                        Toast.makeText(this, s, Toast.LENGTH_SHORT).show();
//                    });
//                    mStorageReference = mFirebaseStorage.getReference(LANGSUNG_DATABASE).child(key)
//                            .child(key + i + "." + getFileExtension(mUri));
//                    sendData(mStorageReference, mUri, key);
//                }
                if (uriList.size() == 0) {
